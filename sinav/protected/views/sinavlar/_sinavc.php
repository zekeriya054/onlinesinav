...
<?php
        ...
        if($ss>0) {
            ...
            echo 'Başarı Düzeyin: % <strong class="yuzde">'.  number_format(($ds*100/$ss),2).'</strong><br><br>';
        }
   //if($k==0) {
   $snc=array();
   $ktg=array();
  // print_r($result);
   ...
   'options'=>array(
      'title' => array('text' => 'Grafiksel Gösterim'),
      'xAxis' => array(
      'categories' => $ktg
      ),
      'yAxis' => array(
      ...
      'title' => false,
      'series' => array(
         array('name' => 'Başarı Düzeyi', 'data' => $snc),
...
echo'</pre>' ;
//}
      echo 'Harcadığın süre:'.$dakika.' <strong class="s1">dakika</strong> '.$saniye.' <strong class="s1">saniye</strong><br>';
        if($ss>0) $seviye=($ds*100)/$ss;
        if(($k+1)<count($result)) {
           if($o_ss>0)
            $o_seviye=($o_ds*100)/$o_ss;
            echo'<h4>Son İki Teste Göre Başarı Gelişimin</h4>';
			echo 'Şimdiki test sonucun: % '.number_format($seviye, 2).'<br>';
		if($o_ss>0)  echo 'Bir önceki test sonucun: % '.number_format($o_seviye, 2).'<br><br>';
            if($seviye>$o_seviye) echo 'Tebrikler, Performansında ARTIŞ Var :) <br><br>';
             else if($seviye==$o_seviye) echo 'Performansın aynı. Kendini geliştirmelisin.! <br><br>';
